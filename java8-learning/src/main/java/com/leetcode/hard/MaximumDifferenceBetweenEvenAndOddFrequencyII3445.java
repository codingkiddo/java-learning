...
public class MaximumDifferenceBetweenEvenAndOddFrequencyII3445 {
		...
		for (char a = '0'; a <= '4'; ++a) {
			for (char b = '0'; b <= '4'; ++b) {
				...
				for (int right = 0; right < n; ++right) {
					...
					// Also, `cnt_b - prev_b >= 2` since b appears an even number of times in the
					// substring,
					// but zero occurrences must be excluded (and 1 must also be excluded since it's
					// odd, obviously)
	...
	/**
	 * Helper function to calculate the 2-bit parity state. Bit 1: parity of cnt_a.
	 * Bit 0: parity of cnt_b.
	 */
